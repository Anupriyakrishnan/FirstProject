...
const ordersuccesspage = async (req, res) => {
    try {
        ...
        if (!userId || !ObjectId.isValid(userId)) {
            console.error("User not authenticated: Invalid or missing user ID in session", userId);
        ...
        const order = await Order.findById(selectedAddress.orderId).populate("orderedItem.Product");
        if (!order || !ObjectId.isValid(order)) {
            console.error(`No valid orderId found in session for user ${userId}`);
            return res.redirect("/cart?message=No recent order found. Please place an order.");
        }
...
const createOrder = async (req, res) => {
  try {
    ...
    let totalAfterOffers = orderedItems.reduce((total, item) => total + (item.totalPrice || 0), 0);
    ...
    if (req.session.appliedCoupon) {
      ...
      if (coupon && totalAfterOffers >= coupon.minimunPrice) {
        ...
        console.log(`✅ Applying coupon ${coupon.name} - Discount: ₹${couponDiscount}`);
      } else {
        if (!coupon) {
          console.warn(`❌ Coupon validation failed - User may have already used this coupon`);
        }
        req.session.appliedCoupon = null;
        couponDiscount = 0;
        appliedCoupon = null;
    ...
    if (appliedCoupon) {
      ...
      console.log(`✅ Coupon ${appliedCoupon.code} marked as used by user ${userId}`);
    }
...
const cancelOrder = async (req, res) => {
    try {
        ...
        const order = await Order.findOne({ orderId }).populate('orderedItem.Product');
        if (!order) {
        ...
        const activeItems = order.orderedItem.filter(it => 
            it.status.toLowerCase() !== 'cancelled' && it.status.toLowerCase() !== 'return'
        );
...
const cancelItem = async (req, res) => {
    try {
        ...
        if (!orderId || !itemId || !reason) {
            console.error(`Invalid input: orderId=${orderId}, productId=${itemId}, reason=${reason}`);
            return res.status(400).json({
                success: false,
    ...
    if (!["pending", "confirmed"].includes(orderStatus)) {
      ...
      return res.status(400).json({
        ...
        if (!item) {
            console.error(`Item not found for productId: ${itemId} in order ${orderId}`);
            return res.status(404).json({
                success: false,
...
const returnOrder = async (req, res) => {
  try {
    ...
    if (!orderId || !reason) {
      return res.status(400).json({ success: false, message: 'Missing orderId or reason' });
    }
...
const returnItem = async (req, res) => {
  try {
    ...
    if (!orderId || !itemId || !reason) {
      console.error(`Invalid input: orderId=${orderId}, itemId=${itemId}, reason=${reason}`);
      return res.status(400).json({
        success: false,
    ...
    if (order.status === 'returnrequest') {
      console.error(`Item-level return blocked due to pending order-level return for order ${orderId}`);
      return res.status(400).json({
        success: false,
        message: 'Item-level returns are disabled due to a pending order-level return request.',
      });
    }
...
const loadwallet = async (req, res) => {
    try {
        ...
        const isCOD = (paymentMethod) => {
            ...
            return pm.includes('cash') && pm.includes('delivery') || pm === 'cod' || pm.includes('cod');
        };
...
//          const itemTotal = Number(item.totalPrice ?? ((item.discountedPrice ?? item.pric) * (item.quantity ?? 1)));

//         if (itemTotal <= 0) continue;

//         if (String(item.status).toLowerCase() === "cancelled") {
//             if (orderIsCOD) {
//                 console.log(`Skipping refund for cancelled COD order ORD:${String(order.orderId).slice(-8)} item ${item._id}`);
//                 continue;
//             }

//             const refundDescription = `Refund for cancelled order ORD:${String(order.orderId).slice(-8)} item:${item._id}`;
...
//                 orderModified = true;
//                 console.log(`Adding refund transaction (cancel): ${refundDescription} amount ${itemTotal}`);
//             } else {
//                 // If wallet already has it but item.refundProcessed not set, set it now to avoid duplicate next time
//                 item.refundProcessed = true;
//                 orderModified = true;
//                 console.log(`Found existing wallet transaction for ${refundDescription}. Marking item refundProcessed.`);
//             }
//         }
...
//             const refundDescription = `Refund for returned order ORD:${String(order.orderId).slice(-8)} item:${item._id}`;
//             const existingTransaction = wallet.transactions.find(
//                 (t) => t.description === refundDescription
//             );
//             if (!existingTransaction) {
...
//                 item.refundProcessed = true;
//                 orderModified = true;
//                 console.log(`Adding refund transaction (return): ${refundDescription} amount ${itemTotal}`);
//             } else {
//                 item.refundProcessed = true;
//                 orderModified = true;
//             }
...
const downloadInvoice = async (req, res) => {
  try {
    const userId = req.session.user?._id || req.session.user;
    if (!userId || !mongoose.Types.ObjectId.isValid(userId)) {
      return res.status(401).json({ success: false, message: "Please log in to download invoice" });
    }
    ...
    const order = await Order.findOne({ orderId, userId }).populate("orderedItem.Product");
    if (!order) {
      return res.status(404).json({ success: false, message: "Order not found" });
    }
    ...
    doc.text(`Invoice Date: ${new Date(order.invoiceDate || Date.now()).toLocaleDateString("en-IN")}`, { align: "right" });
    doc.moveDown();
    ...
    doc.text(`${order.address.addressType || ""}${order.address.landmark ? ", " + order.address.landmark : ""}`);
    ...
    order.orderedItem.forEach((item) => {
      ...
      doc.text(`RS ${Number(item.price).toLocaleString("en-IN")}`, priceX, currentY);
      doc.text(`RS ${Number(item.price * item.quantity).toLocaleString("en-IN")}`, totalX, currentY);
      currentY += 20;
    });

    // Draw table lines
    doc.strokeColor("#000000").lineWidth(1);
    doc.moveTo(itemX, tableTop - 5).lineTo(totalX + 100, tableTop - 5).stroke();
    doc.moveTo(itemX, currentY).lineTo(totalX + 100, currentY).stroke();
    doc.moveTo(itemX, tableTop - 5).lineTo(itemX, currentY).stroke();
    doc.moveTo(totalX + 100, tableTop - 5).lineTo(totalX + 100, currentY).stroke();
    ...
    doc.moveDown();
    doc.fontSize(12);
    doc.text(`Items Total: RS ${Number(order.totalPrice).toLocaleString("en-IN")}`, { align: "right" });
    doc.text(`Discount: RS ${Number(order.discount).toLocaleString("en-IN")}`, { align: "right" });
    doc.text(`Final Amount: RS ${Number(order.finalAmount).toLocaleString("en-IN")}`, { align: "right" });
    doc.moveDown();

    // Footer
    doc.fontSize(10).text("Thank you for shopping with Timeluxe!", { align: "center" });
    doc.text("For any queries, contact us at support@timeluxe.com", { align: "center" });
