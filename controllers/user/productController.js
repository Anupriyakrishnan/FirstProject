        ...
        offers.forEach(offer => {
            offer.productItem.forEach(pi => {
                if (
                    ...
                    mainProductOffer = { discount: pi.discount, offerName: pi.offerName, type: "Product Offer" };
            ...
            offer.categoryItem.forEach(ci => {
                if (
                    ...
                    mainProductOffer = { discount: ci.discount, offerName: ci.offerName, type: "Category Offer" };
            ...
            offers.forEach(offer => {
                ...
                offer.productItem.forEach(pi => {
                    if (
                        ...
                        maxDiscount = pi.discount;
                        relatedOffer = { discount: pi.discount, offerName: pi.offerName, type: "Product Offer" };
                        discountedPrice = relatedProduct.salePrice * (1 - pi.discount / 100);
                    }
                });
                ...
                offer.categoryItem.forEach(ci => {
                    if (
                        ci.category.toString() === relatedProduct.category?._id.toString() &&
                        new Date(ci.startDate) <= currentDate &&
                        new Date(ci.endDate) > currentDate &&
                        ci.discount > maxDiscount
                    ) {
                        maxDiscount = ci.discount;
                        relatedOffer = { discount: ci.discount, offerName: ci.offerName, type: "Category Offer" };
                        discountedPrice = relatedProduct.salePrice * (1 - ci.discount / 100);
                    }
                });
            });
